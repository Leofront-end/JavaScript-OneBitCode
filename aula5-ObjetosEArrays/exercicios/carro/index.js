...
while (true) {
    ...
    if (veiculo.passageiros > veiculo.limiteDepassageiros){
        alert("Altere o numero de limite de passageiros ou de passageiros")
        
    }
    veiculo.velocidade = parseFloat(prompt("Digite a velocidade (em Km/h):"))
    if(veiculo.modelo.length !=0 && veiculo.passageiros.length !=0 && veiculo.limiteDepassageiros.length !=0 && veiculo.velocidade.length !=0 && veiculo.passageiros < veiculo.limiteDepassageiros){
        break
    }
}
